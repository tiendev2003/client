...
import { getDanhMucs, deleteDanhMuc } from './../../../features/danhMucBan/danhMucBanSlice';
import { toast } from 'react-toastify';

export const ManageDanhMucBan = () => {
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Bạn có chắc chắn muốn xóa danh mục này không?")) {
      try {
        await dispatch(deleteDanhMuc(id)).unwrap();
  ...
  const filteredDanhMucs = danhMucs.filter((danhMuc) =>
    danhMuc.TenDMBan.toLowerCase().includes(searchQuery.toLowerCase())
  );
  ...
  return (
    <div className="user-profile-card user-profile-listing">
      <div className="user-profile-card-header">
        <h4 className="user-profile-card-title">Danh mục bàn</h4>
      ...
      <div className="col-lg-12">
        <div className="table-responsive">
          <table className="table text-nowrap">
            ...
            <tbody>
              {currentDanhMucs.map((danhMuc) => (
                <tr key={danhMuc.id}>
                  <td>{danhMuc.id}</td>
                  <td>{danhMuc.TenDMBan}</td>
                  <td>
